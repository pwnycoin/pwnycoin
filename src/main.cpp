...
// Copyright (c) 2013 PwnyCoin developers
...
const string strMessageMagic = "PwnyCoin Signed Message:\n";
...
static const int64 nStartSubsidy = 2048 * COIN;
static const int64 nMinSubsidy = 1 * COIN;

int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = nStartSubsidy;
    if(nHeight < 33) {nSubsidy = 1 * COIN;} //minimal rewards for 1 diff change

		if(nHeight < 8640)	// 1st 3 days x3
		{
		nSubsidy *= 3;
		}
...
//Choose the right diffMode
        ...
        if (TestNet()) {
                if (pindexLast->nHeight+1 >= 33) { DiffMode = 2; } //1st week
        }
        else {
         if (pindexLast->nHeight+1 >= 20160) { DiffMode = 2; } //kimoto
        }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PwnyCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PwnyCoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running PwnyCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PwnyCoinMiner terminated\n");
        throw;
    }
}
